...
public static class AuthenticationHandle
{
    ...
    {
        while (AuthenticationState == AuthenticationState.Authenticated
    ...
    private static async UniTask SignInAnonymouslyAsync(int maxTries)
    {
        ...
        if (AuthenticationState != AuthenticationState.Authenticated)
        {
            Debug.LogWarning($"Player was not singed in successfully after {tries} tries.");
